<script lang="ts">
	import BookCard from '$components/BookCard.svelte';
	import BookCategorie from '$components/BookCategorie.svelte';
	import StarRating from '$components/StarRating.svelte';
	import { getUserState } from '$components/state/user-state.svelte';
	import Icon from '@iconify/svelte';
	let userContext = getUserState();
	let { userName, allBooks } = $derived(userContext);
</script>

<div class="dashboard">
	<div class="dashboard-header mb-m">
		<a href="/private/scan" class="add-book">
			<Icon icon="material-symbols:add-circle-outline" width="80" height="80" />
			<p>Add a book</p>
		</a>
		<div class="headline">
			<h3 class="bold mb-xs">Welcome Back, {userName}</h3>
			<p>
				There's nothing quite like the journey a good book can take you on. Have you discovered any
				new favorites recently ?
			</p>
		</div>
	</div>
	<!-- Book Categories-->
	<BookCategorie
		categorieDescription={'Your Favorite Books'}
		booksToDisplay={userContext.getHighestRatedBooks()}
	></BookCategorie>
	<BookCategorie
		categorieDescription={'Recently added, unread books'}
		booksToDisplay={userContext.getUnreadBooks()}
	></BookCategorie>
	<BookCategorie
		categorieDescription={'Highest rated books from your favorite genre: Fantasy'}
		booksToDisplay={allBooks}
	></BookCategorie>
</div>

<style>
	.dashboard-header {
		display: flex;
		justify-content: space-between;
		align-items: flex-start;
		width: 100%;
	}
	.add-book {
		display: flex;
		align-items: center;
		text-decoration: none;
	}
	.add-book p {
		margin-left: 8px;
	}
	.headline {
		text-align: right;
		max-width: 30%;
		min-width: 300px;
	}
</style>
